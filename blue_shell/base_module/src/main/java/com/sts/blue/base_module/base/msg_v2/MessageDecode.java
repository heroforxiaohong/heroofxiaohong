...
public class MessageDecode extends ByteToMessageDecoder{
    ...
    private final int BASE_LENGTH=4+4+4+32+50+50;//类型 协议头 类型 int+length 4个字节 + actionKey +令牌和 令牌生成时间50个字节
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
        ...
        byte[] actionKeyByte=new byte[32];
        buffer.readBytes(actionKeyByte);
        ...
        head.setActionKey(new String(actionKeyByte).trim());
        Message message=new Message(head, data);
        //认证不通过
        if(!message.authorization(message.buildToken())) {
            ctx.close();
            return;
        }
        out.add(message);
